...
//extern CONST char  gnu_lic[]=GNU_LICENSE;
...
public:
 ...
 CONST char  lcdlogo[]="test serial\n";
int main( void )
{
  ...
  ev.SetTerminal(term);
  term.Init();
//  term.AddCommand(std::shared_ptr<ITerminalCommand>(new mycmd) );
  showcmd.SetSerialIO(serial);
  term.AddCommand(showcmd );
  
 // l.Write(Clcd::LINE1, lcdlogo);
